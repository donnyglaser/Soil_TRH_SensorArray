...
HIH61Donny::HIH61Donny(uint8_t address, uint8_t powerPin)
...
uint8_t HIH61Donny::commandRequest(Stream& stream)
{
  if(stream.available()) {
...
uint8_t HIH61Donny::commandProcess(Stream& stream, uint8_t command)
{
  switch(command) {
  //  get humidity
  case 'h':
